...
module.exports = {
  posts: {
    get(callback) {
      db.any('SELECT post_number,title,url_path FROM posts')
        .then(data => {
          callback(data);
        })
        .catch(error => {
  ...
  post: {
    get(param, callback) {
      db.any('SELECT post_number,title,array_to_json(tags),body,written,edited FROM posts WHERE url_path=$1', param)
        .then(data => {
          callback(data);
        })
        .catch(error => {
